...
def add_file():
    ...
    select_file=askopenfilenames(title="추가되는 이미지 고르기",\
        ...
        initialdir="C:/Users/Administrator/Desktop/파이썬/Game/") #파일열기 대화상장의 초기 경로 설정
...
p_var=DoubleVar()
progress_bar=ttk.Progressbar(frame4, maximum=100, variable= p_var)
...
def merge_image(): #사진을 합치는 함수
    ...
    #가로넓이
    img_width=cbb1.get() # 가로넓이의 콤보박스값 가져오기
    if img_width == "원본유지":
        img_width = -1 # -1일때는 원본 기준으로
    else:     
        img_width = int(img_width) #원본유지가 아니면 가로넓이옵션값을 int로 받아와 사용

    #간격
    img_space = cbb2.get() #간격의 콤보박스값 가져오기
    if img_space == "좁게":
        img_space = 30
    elif img_space == "보통":
        img_space = 60
    ...
    img_format = cbb3.get().lower() # PNG,JPG,BMP 값을 받아와서 소문자로 변경

    
    images=[Image.open(a) for a in listbox.get(0,END)] #리스트박스의 이미지들을 이미지형태로 전부가져오기
    # 이미지 사이즈 리스트에 넣어서 하나씩 처리 [(w,h),(w,h)...]
    image_sizes = []
    if img_width > -1: #이미지가 원본 유지가 아닐때
        image_sizes = [(int(img_width), int(img_width*a.size[1]/a.size[0])) for a in images]
    else:#원본 사이즈 사용
        image_sizes = [(a.size[0],a.size[1]) for a in images]

    #이미지들이 합치게 될 공간(스케치북)만들기
    widths , heights = zip(*(image_sizes))   #zip*을 이용해서 변경된 사이즈 가져오기#############################################
    # 스케치북의 가로크기는 가져온이미지들중 가장 큰 가로크기 와 세로이미지들의 합
    ...
    for  idx, merge_img in enumerate (images):   # 프로그래스바와 연동하기위해 index도 뽑아준다
        # 가로넓이가 원본유지가 아닐때에는 이미지 크기 조정
        if img_width > -1:
            merge_img = merge_img.resize(image_sizes[idx])
        
        
        sketchbook.paste(merge_img, (0,y_offset)) #스케치북에 이미지들을 가져와 (0,y_offset)좌표에 붙이기
        y_offset += (merge_img.size[1] + img_space) #가져온 이미지의 세로크기만큼 y좌표 변경 + 사용자가 지정한 간격

        progress = (idx + 1) / len(images) * 100 # 뽑은 index를 이용하여 프로그레스바의 증가 비율을 만들어준다
        p_var.set(progress) #프로그레스바에 연동시켜주고

        progress_bar.update() #이미지 한개당 비율로 프로그레스바를 업데이트 시켜준다

    
    # 포맷 옵션 처리
    photo_name = "시작에의해 합쳐진사진." + img_format ######################.중요....!!!!
    path = os.path.join(find_file.get(), photo_name) # 저장경로를 불러와 저장될 이미지의 이름과 함께 경로저장
