...
def save():
    ...
    img_format = cbb.get().lower()
    save_name = "시작의 저장." + img_format
    save_path = os.path.join(path.get(), save_name)
    small_img.save(save_path)
    showinfo("알림","저장이 완료되었습니다.")
...
menu_capture.add_command(labe="사진 저장하기",command=save)
